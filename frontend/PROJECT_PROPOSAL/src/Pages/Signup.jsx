...
function Signup() {
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await axios.post("http://localhost:5000/api/otp/verify", { email, otp: otpInput });

      if (role === "student") {
        await axios.post("http://localhost:5000/api/students/save", { email, groupDetails: [] });
        navigate("/student");
      } else if (role === "faculty") {
        await axios.post("http://localhost:5000/api/faculty/create", { email });
        navigate("/faculty");
      }
    } catch (err) {
