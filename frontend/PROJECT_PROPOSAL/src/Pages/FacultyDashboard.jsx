...
function FacultyDashboard() {
  ...
  const fetchTopics = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/topics/${email}`);
      setTopics(res.data);
    } catch (err) { console.log(err); }
  };
  ...
  const fetchProfile = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/faculty/${email}`);
      setFaculty(res.data);
    } catch (err) { console.log(err); }
  };
  ...
  const fetchPendingRequests = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/requests/pending?facultyEmail=${email}`
      );
  ...
  const fetchGroupRequests = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/requests/approved-by-faculty?facultyEmail=${email}`
      );
      const map = {};
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/api/requests/approve/${requestId}`);
  ...
  const handleReject = async (requestId) => {
    setActionLoading(requestId);
    try {
      await axios.patch(`http://localhost:5000/api/requests/reject/${requestId}`);
      setPendingRequests(prev => prev.filter(r => r._id !== requestId));
    } catch (err) {
      console.log(err);
  ...
  const updateProgress = async (studentEmail, field, value) => {
    try {
      await axios.post("http://localhost:5000/api/progress/update-progress", {
  ...
  const deleteTopic = async (id) => {
    await axios.delete(`http://localhost:5000/api/topics/delete/${id}`);
    fetchTopics();
  };
  ...
  const updateTopic = async () => {
    await axios.put(`http://localhost:5000/api/topics/update/${editId}`, {
...
<button
  ...
  onClick={async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/requests/approved?studentEmail=${selectedStudent.email}`
      );
      if (res.data?._id) {
