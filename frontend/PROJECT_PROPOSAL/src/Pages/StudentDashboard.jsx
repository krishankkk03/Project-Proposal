...
function StudentDashboard() {
  ...
  const fetchPendingRequest = useCallback(async (leaderEmail) => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/requests/pending-by-student?studentEmail=${leaderEmail}`
      );
  ...
  const fetchRequestId = useCallback(async (leaderEmail) => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/requests/approved?studentEmail=${leaderEmail}`
      );
  ...
  const fetchStudent = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/students/me?email=${email}`
      );
      if (res.data) {
        setStudent(res.data);
  ...
  const acceptInvitation = async () => {
    try {
      await axios.post("http://localhost:5000/api/students/accept-invitation", {
